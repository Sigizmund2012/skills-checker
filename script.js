...
(function () {
  ...
  function addNewSkill() {
    ...
    // Добавляем навык в индекс навыков (изначально не изучен)
    addSkillToIndex(skill.name, false);
   ...
   */
  function addSkillToIndex(skillName, isLearned = false) {
    const skillsIndexContainer = document.querySelector(
      ".skills-index-sliders"
    );
    ...
    let skillSliderValue = isLearned ? 100 : 0;
    // Дополнительная проверка на localStorage — на случай повторной загрузки
    // (но при первом запуске это не сработает, поэтому выше уже установили значение)
    if (localStorage.getItem("skillsIndex")) {
      const localStorageData = JSON.parse(localStorage.getItem("skillsIndex"));
      if (localStorageData[skillName] !== undefined) {
...
`;
  ...
  function loadSkillsFromStorage() {
    ...
    try {
      ...
      `;
        ...
        addSkillToIndex(skill.name, skill.learned);
      });
  ...
  function addDemoSkillsFromIndex() {
    ...
    // Сначала создаем карточки для демо-навыков
    demoSkills.forEach((skill) => {
      createSkillCard(skill);
    });
    // Затем добавляем слайдеры для всех навыков, передавая статус "изучен"
    demoSkills.forEach((skill) => {
      addSkillToIndex(skill.name, skill.learned);
    });
